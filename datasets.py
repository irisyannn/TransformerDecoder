...
class DataAlg(Dataset):
    ...
    def __getitem__(self, index):
        S = readfile(self.list_items[index])
        max_ns = len(S['xs'])
        if self.train:
...
class DataHumanProbe(Dataset):
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            fn = self.list_items[index] ## if we truly want to iterate over files
            S = readfile(fn) 
...
class DataHumanOpenEnded(Dataset):
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            S = readfile(self.list_items[index]) ## if we truly want to iterate over files
